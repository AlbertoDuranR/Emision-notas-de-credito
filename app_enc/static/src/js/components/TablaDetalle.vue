<template>
  <div class="px-5 mx-auto">
    ...
    >
      ...
      <tbody>
        <tr v-for="item in filteredItems" :key="item.ID_NC">
...
const filteredItems = computed(() => {
  ...
  if(searchFilter.value !== '') {
    const filter = props.listaSolicitudes.filter(item =>
      ...
      item.SOLICITANTE.includes(searchFilter.value)
    );
    console.log('filter', filter)
    return filter
  }
  return props.listaSolicitudes;
});

watchEffect(() => {
      console.log('Valor derivado actualizado:',  filteredItems.value);
    });
